...
　Vuls(VULnerability Scanner)を用いて調査対象サーバ(以下、ターゲット)の情報をJVNやexploit-dbなどに挙げられた脆弱性情報データベースと照らし合わせて、ターゲットが脆弱性を保有しているか調査します。ただし、このソフトウェアの最新のバージョンが現時点でv0.7.0なので完全な動作が保証されていないあるいは開発が試作段階という点を留意して使用しました。
