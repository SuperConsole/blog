...
2019年の目標:「探求し続ける」
## Armitageを用いたGUI上での脆弱性診断(07/10/19)
　Metasploitのためのグラフィカルな便利ツールであるArmitageを用いて手軽に脆弱性診断をしたので記事にまとめます。以前Heartbleedの脆弱性を突くために使用したMetasploitですが、CUIのみで行うにはやや慣れが必要である点と脆弱性モジュールを使用する前にターゲットがどのような脆弱性を持っているかを調査する必要があり、脆弱性をすべて網羅するという点では不向きな面がありました。Armitageでは脆弱性情報のデータベースと照合し、ターゲット上で起動しているアプリケーション(解放ポート)やOSなどの情報を自動で検出し、ターゲットに存在する脆弱性の一覧をGUIで表示する機能があります。これにより、手軽に脆弱性診断を行えるという強みがあるのです。簡単に言うとMetasploitの補助ツール的な立ち位置になります。今回はターゲットとしてMetasploitable2を構築し、いつも通りKaliLinux(攻撃側)からArmitageで診断します。  
  
　方法:  
　　1. KaliLinuxのアプリケーションから「Exploitation Tools -> Armitage」を選択して起動  
　　　//Javaのバージョンは1.7を用いること  
